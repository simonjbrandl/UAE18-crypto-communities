...
comm_summary <- comm_means %>%
  group_by(Location) %>%
  summarize(mdens = mean(Abundance), mric = mean(Speciesrichness))
...
abu.pred.plot <- ggplot(data = abu.pred) +
  ...
  annotate("text", x = 0.15, y = 2.7, size = 3, label = "(b)", color = "black")
abu.pred.plot
...
div.pred.plot <- ggplot(data = div.pred) +
  ...
  annotate("text", x = 0.15, y = 2.7, size = 3, label = "(a)", color = "black")
...
location_centers <- location_hulls %>% group_by(Site) %>% summarize(NMDS1 = mean(NMDS1), NMDS2 = mean(NMDS2))
...
event.abu.pred <- abu.event %>%
  data_grid(Location) %>%
  add_fitted_draws(event.abu.model, n = 1000) %>%
  mutate(pred.density = exp(.value))

event.abu.median <- event.abu.pred %>%
  group_by(Location) %>%
  summarize(median = median(pred.density), sd = sd(pred.density)) %>%
  mutate(se = sd/sqrt(18))
event.abu.median

event.abu.pred.plot <- ggplot(data = event.abu.pred) +
  geom_halfeyeh(aes(y = rev(Location), x = pred.density, fill = Location), color = "black") +
  geom_jitter(aes(x = Density, y = site_intercepts, shape = reorder(Site, sort(as.numeric(site_intercepts))), fill = Location), color = "black", 
             data = abu.event, height = 0.02, 
             alpha = 0.5, size = 0.8) +
  scale_color_manual(values = uaecols) +
  scale_fill_manual(values = alpha(uaecols, 0.5)) +
  scale_shape_manual(name = "Site", values = c(21,22,23,21,22,23)) +
  scale_x_log10(limits = c(0.01,5e3), breaks=c(0,0.01,0.1,1,10,100), labels=c(0,0.01,0.1,1,10,100)) +
  theme_sjb() +
  theme(axis.text.y = element_blank(), 
        axis.ticks.y = element_blank(),
        axis.title.y = element_blank(),
...
event_means <- comms %>% group_by(Location, Site, Sitename, Sitenumber) %>% summarize(Biomass = sum(W), Abundance = n(), Speciesrichness = length(unique(Genspe)), SA = mean(SA)) %>%
  mutate(Density = Abundance/SA,
         Biomass_per_m2 = Biomass/SA,
         Species_density = Speciesrichness/SA,
         site_intercepts = case_when(Site == "Dhabiya" ~ 1.90,
...
sum(table(ctmax$taxloc))

maxmod <- brm(Ctmax ~ taxloc, data = ctmax,  control = list(adapt_delta = 0.995, max_treedepth = 18));beep(sound = "coin")
summary(maxmod)
plot(maxmod)
ctmax.pred <- ctmax %>%
  data_grid(taxloc) %>%  
  add_fitted_draws(maxmod, n = 10000) %>%
...
library(emmeans)
warp_em <- emmeans (maxmod,  ~ taxloc)
cont <- contrast(warp_em, "tukey")
cont
ctmax_means <- ctmax %>%
  group_by(Location, Tax) %>%
  summarize(meantol = mean(Ctmax), sdtol = sd(Ctmax), setol = sdtol/sqrt(n()))
...
ctmin_means <- ctmin %>%
  group_by(Location, Tax) %>%
  summarize(meantol = mean(Ctmin), sdtol = sd(Ctmin), setol = sdtol/sqrt(n()))

sum(table(ctmin$taxloc))

minmod <- brm(Ctmin ~ taxloc, data = ctmin,  control = list(adapt_delta = 0.995, max_treedepth = 18), iter = 5000);beep(sound = "coin")
summary(minmod)

warp_em_min <- emmeans (minmod,  ~ taxloc)
cont <- contrast(warp_em_min, "tukey")
cont

ctmin.pred <- ctmin %>%
  data_grid(taxloc)%>%  
  add_fitted_draws(minmod, n = 10000) %>%
  separate(taxloc, c("Tax", "Location"), remove = F) %>%
  mutate(ordervar = case_when(Tax == "CORYANOM" ~ 6,
                              Tax == "ECSEPULC" ~ 5,
                              Tax == "ENNEVENT" ~ 4,
...
network.coi <- coi.pa %>%
  ...
  group_by(ID, Tax, Location) %>%
  summarize(value = sum(value)) %>%
  mutate(LocTax = interaction(Location, Tax)) %>%
  select(ID, LocTax, everything())

cfstat.coi <- graph_from_data_frame(network.coi)

###get network community characteristics
ed.coi <- edge_density(cfstat.coi, loops=F)

net.sym.coi <- as.undirected(cfstat.coi, mode= "collapse", edge.attr.comb=list(weight="sum", "ignore"))

ceb.coi <- cluster_edge_betweenness(net.sym.coi) 
memb.coi <- membership(ceb.coi)
mod.coi <- modularity(ceb.coi)

###permutations

mod.perm.coi <- data.frame(ed_perm = numeric(length = 99), mod_perm = numeric(length = 99), 
                           AG.CORYANOM = numeric(length = 99), AG.ECSEPULC = numeric(length = 99), AG.ENNEVENT = numeric(length = 99), 
                           GoO.CORYANOM = numeric(length = 99), GoO.ECSEPULC = numeric(length = 99), GoO.ENNEVENT = numeric(length = 99))

for (i in 1:99) {
  print(i)

  
meta.coi.random <- meta.coi %>%
  select(Tax, Location) %>%
  sample_frac(1L) %>%
  add_column(ID.match = meta.coi$ID.match) %>%
  select(ID.match, Location, Tax)

network.coi.rand <- coi.pa %>%
  select(-rowsum) %>%
  pivot_longer(cols = 2:547, names_to = "ID") %>%
  filter(value > 0) %>%
  left_join(meta.coi.random[1:3], by = "ID.match") %>%
  group_by(ID, Location, Tax) %>%
  summarize(value = sum(value)) %>% 
  mutate(LocTax = interaction(Location, Tax)) %>%
  select(ID, LocTax, everything())

cfstat.coi.rand <- graph_from_data_frame(network.coi.rand)

###get network community characteristics
ed.coi.rand <- edge_density(cfstat.coi.rand, loops=F)

net.sym.coi.rand <- as.undirected(cfstat.coi.rand, mode= "collapse", edge.attr.comb=list(weight="sum", "ignore"))

ceb.coi.rand <- cluster_edge_betweenness(net.sym.coi.rand) 
memb.coi.rand <- membership(ceb.coi.rand)
mod.coi.rand <- modularity(ceb.coi.rand)

df <- memb.coi.rand[513:518]
df1 <- df[order(names(df))]

mod.perm.coi[i,1] <- ed.coi.rand
mod.perm.coi[i,2] <- mod.coi.rand
mod.perm.coi[i,3] <- df1[1]
mod.perm.coi[i,4] <- df1[2]
mod.perm.coi[i,5] <- df1[3]
mod.perm.coi[i,6] <- df1[4]
mod.perm.coi[i,7] <- df1[5]
mod.perm.coi[i,8] <- df1[6]
}



mod.perm.comp.coi <- mod.perm.coi %>%
  mutate(ed_emp = ed.coi,
         mod_emp = mod.coi,
         ed_comp = case_when(ed_perm < ed_emp ~ 1,
                             TRUE ~ 0),
         mod_comp = case_when(mod_perm > mod_emp ~ 1,
                              TRUE ~ 0))

mod.perm.comp.coi

groupings <- mod.perm.comp.coi %>%
  pivot_longer(cols = c(3:8), names_to = "cluster", values_to = "group") %>%
  separate(cluster, c("Location", "Species"), remove = F) %>%
  mutate(iteration = rep(1:99, each = 6)) +
  group_by

group.plot <- ggplot(groupings, aes(x = iteration, y = group, color = Location, shape =  Species)) +
...
ggsave("Plots/FigSupp1Groups.pdf", group.plot, width = 6, height = 6, useDingbats = F)
...
network.plot.coi <- ggplot(data = network.coi.colored) +
  geom_net(layout.alg = "fruchtermanreingold", aes(from_id = ID, to_id = taxloc, colour = share.color, shape = Tax, alpha = 0.5),
  lwd=3, labelon = F, ealpha = 0.3, arrowsize = 0.1, linewidth = 0.2, 
  vjust = 0.5, hjust = 0.5) +
...
ggsave("Plots/nwtcoi_both.pdf", plot = network.plot.coi, width = 8, height = 8, useDingbats=FALSE)


share.color <- network.coi.rand %>%
  select(ID, Location) %>%
  distinct() 
...
network.coi.colored.rand <- network.coi.rand %>%
  left_join(shared, by = "ID") %>%
  mutate(taxloc = interaction(Tax, Location.y))

uaecols.network <- c(uaecols, "grey69")

network.plot.coi.rand <- ggplot(data = network.coi.colored.rand) +
  geom_net(layout.alg = "fruchtermanreingold", aes(from_id = ID, to_id = taxloc, 
                                                   colour = share.color, fill = share.color, shape = Tax),
  lwd=3, labelon = F, ealpha = 0.3, arrowsize = 0.1, linewidth = 0.2, 
  vjust = 0.5, hjust = 0.5) +
  # labelcolour = cfnet2$lcolour) +
  theme_net() +
  ...
  scale_shape_manual(values = c(15:17)) +
  theme(legend.position = "right")
network.plot.coi.rand


############
meta.23s <- sub23s.loc[c(1:3)] %>%
  filter(Tax %in% c("ECSEPULC", "ENNEVENT", "CORYANOM"))
...
network.plot.23s <- ggplot(data = network.23s.colored) +
  geom_net(layout.alg = "fruchtermanreingold", aes(from_id = ID, to_id = taxloc, colour = share.color, shape = Tax, alpha = 0.3),
  lwd=3, labelon = F, ealpha = 0.1, arrowsize = 0.1, linewidth = 0.2, 
...
ggsave("Plots/nwt23s_both.pdf", plot = network.plot.23s, width = 8, height = 8, useDingbats=FALSE)

networklegend<-g_legend(network.plot.23s)

networkplots <- gbm::grid.arrange(networklegend, gridExtra::arrangeGrob(network.plot.coi + theme(legend.position="none"),
...
###########################
####### GoO only ##########
meta.coi.goo <- coi.sub.taxloc[c(1:3)]
network.coi.goo <- coi.pa.taxloc %>%
  select(-rowsum) %>%
  pivot_longer(cols = 2:547, names_to = "ID") %>%
  filter(value > 0) %>%
  left_join(meta.coi.goo[1:3], by = "ID.match") %>%
  group_by(ID, Location, Tax) %>%
  summarize(value = sum(value)) %>%
  filter(Location == "GoO") %>%
  ungroup() %>%
...
#network analysis
cfstat.coi.goo <- graph_from_data_frame(network.coi.goo)

###get network community characteristics
ed.coi.goo <- edge_density(cfstat.coi.goo, loops=F)

net.sym.coi.goo <- as.undirected(cfstat.coi.goo, mode= "collapse", edge.attr.comb=list(weight="sum", "ignore"))

ceb.coi.goo <- cluster_edge_betweenness(net.sym.coi.goo) 
memb.coi.goo <- membership(ceb.coi.goo)
modul.goo.coi <- modularity(ceb.coi.goo)



table(network.coi.goo$ID, network.coi.goo$Tax)

share.color <- network.coi.goo %>%
  ungroup() %>%
  select(ID, Tax) %>%
  distinct() 
...
shared <- share.color %>%
  ...
  mutate(sharedornot = case_when(shared == 1 ~ "unique",
                                 TRUE ~ "Shared")) %>%
  right_join(share.color) %>%
  mutate(share.color = case_when(sharedornot == "unique" ~ paste(Tax),
                                 TRUE ~ paste(sharedornot)))
  
network.coi.goo.colored <- network.coi.goo %>%
  left_join(shared, by = "ID") 

goo.cols <- fish(n = 6, option = "Trimma_lantana", begin = 0.8, end = 0.2)
uaecols.network.goo <- c(goo.cols, "grey46")

network.plot.coi.goo <- ggplot(data = network.coi.goo.colored) +
  geom_net(layout.alg = "fruchtermanreingold", aes(from_id = ID, to_id = Tax.x, colour = share.color, shape = sharedornot, alpha = 0.5),
  lwd=3, labelon = F, ealpha = 0.1, arrowsize = 0.1, linewidth = 0.2, 
  vjust = 0.5, hjust = 0.5) +
  # labelcolour = cfnet2$lcolour) +
  theme_net() +
  # scale_colour_manual(values = c("deepskyblue", "navy", "mediumspringgreen", "green4")) +
  scale_color_manual(values = uaecols.network.goo) +
  scale_shape_manual(values = c(15:16)) +
  theme(legend.position = "right")
network.plot.coi.goo


ggsave("Plots/nwtcoi_goo.pdf", plot = network.plot.coi.goo, width = 8, height = 8, useDingbats=FALSE)


###########permutation CO
meta.coi.goo = meta.coi %>%
  filter(Location == "GoO") %>%
  select(-Location) 

mod.perm <- data.frame(ed_perm = numeric(length = 99), mod_perm = numeric(length = 99))

for (i in 1:999) {
  print(i)
meta.coi.goo.random <- meta.coi.goo %>%
  select(Tax) %>%
  sample_frac(1L) %>%
  add_column(ID.match = meta.coi.goo$ID.match)

network.coi.goo.rand <- coi.pa.taxloc %>%
  select(-rowsum) %>%
  pivot_longer(cols = 2:547, names_to = "ID") %>%
  filter(value > 0) %>%
  right_join(meta.coi.goo.random[1:2], by = "ID.match") %>%
  group_by(ID, Tax) %>%
  summarize(value = sum(value))

cfstat.coi.goo.rand <- graph_from_data_frame(network.coi.goo.rand)

###get network community characteristics
ed.coi.goo.rand <- edge_density(cfstat.coi.goo.rand, loops=F)

net.sym.coi.goo.rand <- as.undirected(cfstat.coi.goo.rand, mode= "collapse", edge.attr.comb=list(weight="sum", "ignore"))

ceb.coi.goo.rand <- cluster_edge_betweenness(net.sym.coi.goo.rand) 
memb.coi.goo.rand <- membership(ceb.coi.goo.rand)
modul.coi.goo.rand <- modularity(ceb.coi.goo.rand)

mod.perm[i,1] <- ed.coi.goo.rand
mod.perm[i,2] <- modul.coi.goo.rand
}

mod.perm.comp <- mod.perm %>%
  mutate(ed_emp = ed.coi.goo,
         mod_emp = modul.goo.coi,
         ed_comp = case_when(ed_perm > ed_emp ~ 1,
                             TRUE ~ 0),
         mod_comp = case_when(mod_perm < mod_emp ~ 1,
                              TRUE ~ 0))
...
###########GOO 23S#####################
#######################################
meta.23s <- sub23s.taxloc[c(1:3)]
network.23s.goo <- pa23s.taxloc %>%
  select(-rowsum) %>%
  pivot_longer(cols = 2:3007, names_to = "ID") %>%
  filter(value > 0) %>%
  left_join(meta.23s[1:3], by = "ID.match") %>%
...
table(network.23s.goo$ID, network.23s.goo$Tax)

share.color <- network.23s.goo %>%
  ungroup() %>%
  select(ID, Tax) %>%
  distinct() 
...
network.23s.goo.colored <- network.23s.goo %>%
  left_join(shared, by = "ID") 

goo.cols <- fish(n = 6, option = "Trimma_lantana", begin = 0.8, end = 0.2)
uaecols.network.goo <- c(goo.cols, "grey69")

network.plot.23s.goo <- ggplot(data = network.23s.goo.colored) +
  geom_net(layout.alg = "fruchtermanreingold", aes(from_id = ID, to_id = Tax.x, colour = share.color, shape = sharedornot, alpha = 0.3),
  lwd=3, labelon = F, ealpha = 0.1, arrowsize = 0.1, linewidth = 0.2, 
  vjust = 0.5, hjust = 0.5) +
  # labelcolour = cfnet2$lcolour) +
  theme_net() +
  ...
  scale_shape_manual(values = c(15:16)) +
  theme(legend.position = "right")
network.plot.23s.goo

ggsave("Plots/nwt23s_goo.pdf", plot = network.plot.23s.goo, width = 8, height = 8, useDingbats=FALSE)


cfstat.23s.goo <- graph_from_data_frame(network.23s.goo)

###get network community characteristics
ed.23s.goo <- edge_density(cfstat.23s.goo, loops=F)

net.sym.23s.goo <- as.undirected(cfstat.23s.goo, mode= "collapse", edge.attr.comb=list(weight="sum", "ignore"))

ceb.23s.goo <- cluster_edge_betweenness(net.sym.23s.goo) 
memb.23s.goo <- membership(ceb.23s.goo)
modul.23s.goo <- modularity(ceb.23s.goo)
memb.23s.goo[1925:1930]
...
###########################
####### GoO only ##########
meta.coi.all <- coi.sub.taxloc[c(1:3)]
...
network.coi.plot <- coi.pa.taxloc %>%
  select(-rowsum) %>%
  pivot_longer(cols = 2:547, names_to = "ID") %>%
  filter(value > 0) %>%
  left_join(meta.coi.all[1:3], by = "ID.match") %>%
...
network.coi.plot.comb <- network.coi.plot %>%
  left_join(shared, by = "ID") %>%
  mutate(taxloc = interaction(Tax, Location.y))
...
prey.mod.csv <- read.csv(file = "Data/COImembership_20iter.csv") %>%
  select(-X)

module.tree <- network.coi.plot %>%
  inner_join(prey.mod.csv, by = c("ID.match", "Location", "Tax"))


share.color <- module.tree %>%
  ungroup() %>%
  select(ID, Location) %>%
  distinct() 
...
network.coi.plot.comb <- module.tree %>%
  left_join(shared, by = "ID") 
...
helper <- pred.modularity %>%
  arrange(ID.match, taxloc)
###visualize clusters
...
###23s
meta.23s.all <- sub23s.taxloc[c(1:3)]
...
length(unique(network.23s.all$ID))

network.23s.plot <- pa23s.taxloc %>%
  select(-rowsum) %>%
  pivot_longer(cols = 2:3007, names_to = "ID") %>%
  filter(value > 0) %>%
  left_join(meta.23s.all[1:3], by = "ID.match") %>%
...
share.color.23s <- network.23s.plot %>%
  select(ID, Location) %>%
  distinct() 
...
network.23s.colored <- network.23s.plot %>%
  left_join(shared.23s, by = "ID") %>%
  mutate(taxloc = interaction(Tax, Location.y))
...
network.plot.23s <- ggplot(data = network.23s.colored) +
 geom_net(layout.alg = "fruchtermanreingold", aes(from_id = ID, to_id = taxloc, colour = share.color, shape = Tax, alpha = 0.75),
  lwd=3, labelon = F, ealpha = 0.1, arrowsize = 0.1, linewidth = 0.2, 
  vjust = 0.5, hjust = 0.5) +
  # labelcolour = cfnet2$lcolour) +
  theme_net() +
...
prey.mod.23s <- read.csv(file = "Data/23smembership_20iter.csv") %>%
  select(-X)

module.tree.23s <- network.coi.plot %>%
  inner_join(prey.mod.23s, by = c("ID.match", "Location", "Tax"))
...
shared.23s <- share.color.23s %>%
  ...
  mutate(sharedornot = case_when(shared == 1 ~ "unique",
                                 TRUE ~ "Shared")) %>%
  right_join(share.color.23s) %>%
  mutate(share.color.23s = case_when(sharedornot == "unique" ~ paste(Location),
                                 TRUE ~ paste(sharedornot)))
...
mean(environment.vars$BO_sstmean)
...
maxplot <- ggplot(data = mapdata.max, aes(x = x, y = y)) +
  ...
  xlab("Longitude") +
  ylab("Latitude") +
  annotate("text", x = 53.5, y = 26, size = 2, label = "ARABIAN GULF", color = "black", fontface = 2) +
  annotate("text", x = 58.5, y = 24.5, size = 2, label = "GULF OF OMAN", color = "black", fontface = 2) +
  annotate("text", x = 52, y = 30, size = 2, label = "a) Maximum", color = "black", fontface = 2) +
...
minplot <- ggplot(data = mapdata.min, aes(x = x, y = y)) +
  ...
  annotate("text", x = 53.5, y = 26, size = 2, label = "ARABIAN GULF", color = "black", fontface = 2) +
  annotate("text", x = 58.5, y = 24.5, size = 2, label = "GULF OF OMAN", color = "black", fontface = 2) +
  annotate("text", x = 52, y = 30, size = 2, label = "b) Minimum", color = "black", fontface = 2) +
  coord_cartesian(clip="off")
minplot
...
rangeplot <- ggplot(data = mapdata.range, aes(x = x, y = y)) +
  ...
  annotate("text", x = 53.5, y = 26, size = 2, label = "ARABIAN GULF", color = "black", fontface = 2) +
  annotate("text", x = 58.5, y = 24.5, size = 2, label = "GULF OF OMAN", color = "black", fontface = 2) +
  annotate("text", x = 52, y = 30, size = 2, label = "c) Range", color = "black", fontface = 2) +
  coord_cartesian(clip="off")
rangeplot

temp_plots <- ggarrange(maxplot, minplot, rangeplot, ncol = 3, nrow = 1)
temp_plots
ggsave("Plots/temperature_plots.png", temp_plots, width = 8, height = 3)
...
###run mds
benth.mds <- metaMDS(benth.tidy[-c(1:4)], metric = "Bray", trymax = 1000)
benth.mds
...
point.plot.benth <- ggplot(mds.points.benth) +
  ...
  geom_segment(aes(x=0, xend=NMDS1, y=0, yend=NMDS2),
               ...
               inherit.aes = F) +
  theme(legend.position = "top") +
  geom_text_repel(aes(x=NMDS1, NMDS2, label=category), size=3, inherit.aes = F) +
  scale_y_continuous(limits = c(-0.75, 0.75), breaks = seq(-0.75, 0.75, 0.25)) +
  scale_x_continuous(limits = c(-1.2, 1.2)) +
  xlab("NMDS1") +
  ylab("NMDS2")
...
###run convex hulls on habitats
site_hulls <- ddply(site.scores.benth, "location", convex_hull)


###create vector with habitat names in center of convex hulls
site_centers <- site_hulls %>% group_by(location) %>% summarize(NMDS1 = mean(NMDS1), NMDS2 = mean(NMDS2))
