...
comm_summary <- comm_means %>%
  group_by(Location) %>%
  summarize(mdens = mean(Abundance/SA), mric = mean(Speciesrichness/SA), mbiom = mean(Biomass_per_m2/SA),
            sddens = sd(Abundance/SA), sdric = sd(Speciesrichness/SA), sdbiom = sd(Biomass_per_m2/SA),
            n = n(),
