...
abu.pred <- comm_means %>%
  data_grid(Location) %>%
  add_fitted_draws(abundance.model, n = 1000) %>%
...
bio.pred <- comm_means %>%
  data_grid(Location) %>%
  add_fitted_draws(biomass.model, n = 1000) %>%
  mutate(pred.biomass = exp(.value))
...
div.pred <- comm_means %>%
  data_grid(Location) %>%
  add_fitted_draws(diversity.model, n = 1000) %>%
  mutate(pred.diversity = exp(.value))
...
ggsave("Plots/Fig1_MS.png", Fig1, width = 6, height = 5)
```
...
network.plot.coi.goo <- ggplot(data = network.coi.goo.colored) +
  geom_net(layout.alg = "fruchtermanreingold", aes(from_id = ID, to_id = Tax.x, colour = share.color, shape = sharedornot),
  lwd=3, labelon = F, alpha = 0.8, ealpha = 0.5, arrowsize = 0.1, linewidth = 0.05, 
  vjust = 0.5, hjust = 0.5) +
  # labelcolour = cfnet2$lcolour) +
  theme_net() +
  # scale_colour_manual(values = c("deepskyblue", "navy", "mediumspringgreen", "green4")) +
...
perm.benth = adonis(benth.tidy[-c(1:4)] ~ location, benth.tidy, distance = "manhattan", permutations = 999)
