...
ggsave("Plots/Fig1_MS.pdf", Fig1, width = 8, height = 12, useDingbats = F)
```
...
comp_plot
ggsave("Plots/Fig2_MS.pdf", plot = comp_plot, width = 8, height = 6, useDingbats=FALSE)
```
...
event.plot <- ggplot(size.preds.event, aes(x = TL, y = W, color = ordered(Location))) +
  ...
  geom_point(data = size.strs.event, aes(fill = Location), alpha = 0.8, size = 1, shape = 23, color = "black") +
  scale_color_manual(values = uaecols) +
  scale_fill_manual(values = uaecols) +
...
canom.plot <- ggplot(size.preds.canom, aes(x = TL, y = W, color = ordered(Location))) +
  geom_line(aes(y = .value, group = paste(Location, .draw)), alpha = .3, lwd = 0.1) +
  geom_point(data = size.strs.canom, aes(fill = Location), size = 1, alpha = 0.8, shape = 23, color = "black") +
  scale_color_manual(values = uaecols) +
  scale_fill_manual(values = uaecols) +
  ylab("body weight in g") +
  xlab("total length (TL) in mm") +
...
ggsave("Plots/Fig3_MS.pdf", Fig3, width = 4, height = 7, useDingbats = F)
...
ctmax.pred.plot <- ggplot(data = ctmax.pred) +
  ...
  geom_jitter(aes(x = Ctmax, y = ordervar, shape = Tax, fill = Location),
             data = ctmax, size = 1.5, color = "black", shape = 23, height = 0.1) +
  ...
  annotate("text", x = 34.5, y = 6.15, 
    ...
    annotate("text", x = 34, y = 7, size = 5, label = "(b)")
...
ctmin.pred.plot <- ggplot(data = ctmin.pred) +
  ...
  geom_jitter(aes(x = Ctmin, y = ordervar, shape = Tax, fill = Location),
             data = ctmin, size = 1.5, color = "black", shape = 23, height = 0.1) +
  scale_color_manual(values = uaecols, labels = c("Arabian Gulf", "Gulf of Oman")) +
  scale_fill_manual(values = alpha(uaecols, 0.9), labels = c("Arabian Gulf", "Gulf of Oman")) +
    theme_bw() +
  theme(axis.title.y = element_blank(),
...
ggsave("Plots/Fig4_MS.pdf", plot = ctplots, width = 12, height = 8, useDingbats=FALSE)
```
...
ggsave("Plots/Fig6_MS.pdf", plot = rarefactionplots, width = 6, height = 10, useDingbats=FALSE)
```
...
my.sites.environment <- data.frame(Name=my.sites$Name , extract(environment.bottom,my.sites[,2:3]))

comms.post <- comms %>%
  unite("Name", c(Location, Site), remove = FALSE) %>%
  mutate(LinfTL = TL/10) %>%
  left_join(environment.vars, by = "Name") %>%
  rename(sstmean = BO2_tempmean_bdmean)
