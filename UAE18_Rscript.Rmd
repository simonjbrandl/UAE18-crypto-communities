...
ctmax.pred.plot <- ggplot(data = ctmax.pred) +
  ...
  annotate("text", x = 34.5, y = 6.15, 
            label = 'italic("C. anomolus")', size = 2.5, parse = TRUE) +
...
ggsave("Plots/Fig3_MS.pdf", plot = ctplots, width = 8, height = 7)
```
...
mod23s <- modularity(ceb23s)
memb23s[2257:2262]
###permutations

mod.perm.23s <- data.frame(ed_perm = numeric(length = 99), mod_perm = numeric(length = 99))

for (i in 1:99) {
  print(i)

  
meta.23s.random <- meta.coi %>%
  select(Tax, Location) %>%
  sample_frac(1L) %>%
  add_column(ID.match = meta.coi$ID.match) %>%
  select(ID.match, Location, Tax)

network.23s.rand <- pa23s.loc %>%
  select(-rowsum) %>%
  pivot_longer(cols = 2:3007, names_to = "ID") %>%
  filter(value > 0) %>%
  right_join(meta.23s.random[1:3], by = "ID.match") %>%
  group_by(ID, Location, Tax) %>%
  summarize(value = sum(value))

cfstat.23s.rand <- graph_from_data_frame(network.23s.rand)

###get network community characteristics
ed.23s.rand <- edge_density(cfstat.23s.rand, loops=F)

net.sym.23s.rand <- as.undirected(cfstat.23s.rand, mode= "collapse", edge.attr.comb=list(weight="sum", "ignore"))

ceb.23s.rand <- cluster_edge_betweenness(net.sym.23s.rand) 
memb.23s.rand <- membership(ceb.23s.rand)
mod.23s.rand <- modularity(ceb.23s.rand)

mod.perm.23s[i,1] <- ed.23s.rand
mod.perm.23s[i,2] <- mod.23s.rand
}

mod.perm.comp.23s <- mod.perm.23s %>%
  mutate(ed_emp = ed23s,
         mod_emp = mod23s,
         ed_comp = case_when(ed_perm < ed_emp ~ 1,
                             TRUE ~ 0),
